...
class CommandHandler {
  constructor() {
    ...
    console.log('Comandos administrativos cargados:', Object.keys(this.commands).join(', '));
   ...
   */
  isCommand(message) {
    return message.startsWith('/') && Object.keys(this.commands).includes(message.split(' ')[0]);
  }
  ...
  isChatDisabled(chatId) {
    ...
    const isDisabledByCommand = this.disabledChats[chatId] && 
                              this.disabledChats[chatId].expiry > Date.now();
  
    // Verificar si está desactivado por intervención manual
    const isDisabledByIntervention = this.manualInterventions[chatId] && 
                                   this.manualInterventions[chatId].expiry > Date.now();
  
    // Verificar si está desactivado por proceso completado
    const isDisabledByCompletion = this.completedChats[chatId] && 
                                 this.completedChats[chatId].expiry > Date.now();
  
    return isDisabledByCommand || isDisabledByIntervention || isDisabledByCompletion;
  }
  ...
  detectOwnerMessage(chatId, message) {
    ...
    console.log(`Dueño ha intervenido en ${chatId}. Bot desactivado por 24 horas.`);
    return true;
  }
  ...
  registerManualIntervention(chatId, hours = 24) {
    ...
    this.saveDisabledState();
    console.log(`Intervención manual registrada en ${chatId} por ${hours} horas`);
  }
  ...
  markProcessCompleted(chatId, hours = 24) {
    ...
    this.saveDisabledState();
    console.log(`Chat ${chatId} marcado como completado. Bot desactivado por ${hours} horas para este chat.`);
  }
  ...
  async handleCommand(sock, message) {
    ...
    const fullCommand = message.message.conversation || 
    ...
    if (this.commands[command]) {
      try {
        ...
        await sendBotMessage(sock, chatId, `❌ Error al ejecutar el comando: ${error.message}`);
      }
    }
  }
  ...
  async deactivateBot(sock, chatId, args) {
    ...
    if (isNaN(hours) || hours <= 0 || hours > 168) {
      ...
      // });
      await sendBotMessage(sock, chatId, '❌ Por favor especifica un número válido de horas (1-168)');
      return;
    }
    ...
    // });
    await sendBotMessage(sock, chatId, `✅ Bot desactivado por ${hours} horas. Se reactivará automáticamente el ${offUntil}` );
  }
  ...
  async activateBot(sock, chatId) {
    ...
    if (wasDisabled) {
      ...
      await sendBotMessage(sock, chatId, '✅ Bot activado nuevamente para este chat');
    } else {
      // await sock.sendMessage(chatId, { text: '✅ El bot ya estaba activo para este chat' });
      await sendBotMessage(sock, chatId, '✅ El bot ya estaba activo para este chat');
    }
  }
  ...
  async checkStatus(sock, chatId) {
    ...
    if (this.disabledChats[chatId]) {
      const expiryDate = new Date(this.disabledChats[chatId].expiry).toLocaleString();
      status += `⚠️ *Desactivado* por comando hasta: ${expiryDate}\n`;
    } 
    else if (this.manualInterventions[chatId]) {
      const expiryDate = new Date(this.manualInterventions[chatId].expiry).toLocaleString();
      status += `🧑‍💼 *Intervención manual* registrada hasta: ${expiryDate}\n`;
    }
    else if (this.completedChats[chatId]) {
      const expiryDate = new Date(this.completedChats[chatId].expiry).toLocaleString();
  ...
  async getStats(sock, chatId) {
    ...
    const statsMessage = `📊 *Estadísticas del Bot*\n\n` +
      ...
      `Usuarios sin idioma definido: ${totalUsers - spanishUsers - englishUsers}\n\n` +
      `Activos hoy: ${activeToday}\n` +
      `Activos últimos 7 días: ${activeLast7Days}\n\n` +
      `Chats desactivados: ${Object.keys(this.disabledChats).length}\n` +
      `Intervenciones manuales: ${Object.keys(this.manualInterventions).length}\n` +
  ...
  async manageReactivation(sock, chatId, args) {
    ...
    if (!args.length) {
      ...
      const statusMsg = `🔄 *Estado del Sistema de Reactivación*\n\n` +
        `• Estado: ${status.running ? '✅ Activo' : '❌ Inactivo'}\n` +
        `• Intervalo: ${status.checkIntervalMinutes} minutos\n` +
        `• Última verificación: ${status.lastCheck ? new Date(status.lastCheck).toLocaleString() : 'Nunca'}\n\n` +
        `• Pendientes de reactivación: ${status.pendingReactivations.disabledChats + 
