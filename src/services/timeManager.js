 ...
 * MODIFICADO: Ahora usa la hora de Miami, Florida (Eastern Time)
 */
...
class TimeManager {
  ...
  /**
   * Obtiene la fecha y hora actual o simulada en la zona horaria de Miami
   * @returns {Date} Fecha y hora actual o simulada en la zona horaria de Miami
   */
  getCurrentDate() {
    if (this.mockDate) {
    ...
    const miamiTime = new Date(now.toLocaleString('en-US', { timeZone: this.timeZone }));
  ...
  /**
   * Formatea una fecha en string legible con la zona horaria de Miami
  ...
  /**
   * Método de debug para verificar la hora actual en diferentes zonas horarias
   * @returns {Object} Información de debug sobre las horas actuales
   */
  debugTimezones() {
    const now = new Date();
    return {
      utc: now.toISOString(),
      miami: now.toLocaleString('en-US', { timeZone: this.timeZone }),
      local: now.toLocaleString(),
      businessHours: this.isBusinessHours(),
      currentHour: this.getCurrentDate().getHours()
    };
  }
}
